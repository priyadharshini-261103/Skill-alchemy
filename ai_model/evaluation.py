...
def load_data():
    ...
    try:
        with engine.connect() as conn:
            ...
            courses = pd.read_sql("SELECT course_id, course_name, category, difficulty FROM courses", conn)
...
def assign_learning_style(preference):
    """ Assigns learning style based on user preferences. """
    if "visual" in preference.lower():
        return "Visual"
    elif "auditory" in preference.lower():
        return "Auditory"
    elif "kinesthetic" in preference.lower():
        return "Kinesthetic"
    return "Mixed"
...
def update_user_data(processed_data):
    ...
    try:
        with engine.begin() as conn:
            ...
            if data_tuples:
                conn.execute(update_query, data_tuples)
